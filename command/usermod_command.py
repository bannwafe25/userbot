...
async def mping_cmd(client, message):
    ...
    matches = re.findall(r"(?:time|waktu)=([\d.]+)\s?ms", out)
    ping_result = matches[-1] if matches else "0.0"
    upnya = await get_time((time() - start_time))
...
async def unblock_cmd(client, message):
    ...
    text_arg = message.command[1] if len(message.command) > 1 else ""
    user_id = await client.extract_userid(message, text_arg)
    ...
    try:
        ...
        text = f"{emo.gagal}<b>I have never interacted with <code>{user_id}</code></b>"
        return await pros.edit(text) if pros else await message.reply(text)
    except Exception as e:
        text = f"{emo.gagal}<b>Error:</b>\n<code>{str(e)}</code>"
        return await pros.edit(text) if pros else await message.reply(text)
        
    if user.id == client.me.id:
        text = f"{emo.sukses}<b>Ok!</b>"
        return await pros.edit(text) if pros else await message.reply(text)
        
    await client.unblock_user(user.id)
    
    text = f"{emo.sukses}<b>Successfully unblocked {user.mention}.</b>"
    return await pros.edit(text) if pros else await message.reply(text)
...
async def block_cmd(client, message):
    ...
    if message.reply_to_message:
        ...
        user_target = message.text.split(" ", 1)[1].strip()
        if user_target.isdigit():
            user_id = int(user_target)
        else:
            try:
                chat = await client.get_chat(user_target)
                user_id = chat.id
            except Exception as e:
                return await message.reply(f"{emo.gagal}<b>Gagal menemukan pengguna:</b>\n<code>{str(e)}</code>")
    ...
    try:
        ...
        return await pros.edit(f"{emo.gagal}<b>Gagal memblokir:</b>\n<code>{str(e)}</code>")
