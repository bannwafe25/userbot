...
async def ucast_cmd(client, message):
    ...
    if not text:
        return await proses.edit(
            f"{em.gagal}<code>{message.text.split()[0]}</code> <b>teks atau reply diperlukan</b>"
        )

    task_id = task.start_task()
    prefix = client.get_prefix(client.me.id)
    
    state_key = f"inline_cancel {task_id} {id(message)}"
    ...
    try:
        ...
        await proses.edit(
            f"{em.proses}<i>Broadcast sedang berjalan #<code>{task_id}</code>.\n"
            f"Ketik <code>{prefix[0]}cancel {task_id}</code> untuk membatalkan!</i>"
        )
    ...
    try:
        for chat_id in chats:
            ...
            if chat_id in (blacklist or []) or chat_id in (BLACKLIST_GCAST or []) or chat_id in (DEVS or []):            
                continue
            ...
            try:
                ...
                failed += 1
                error_log.append(f"{chat_id}: PeerFlood (Limit Kirim)")
                continue

            except Exception as err:
                failed += 1
                error_log.append(f"{chat_id}: {str(err)}")
        ...
        try:
            data_todelete = state.get(state_key)
            if data_todelete and isinstance(data_todelete, dict):
                chatid = int(data_todelete.get("chat"))
                msgid = int(data_todelete.get("_id"))
                await client.delete_messages(chatid, msgid)
        except Exception as e:
            logger.error(f"Gagal menghapus inline message: {e}")
        ...
        try:
            await client.send_message(
                ...
                f"Gunakan <code>,bc-error</code> untuk melihat detail gagal."
            )
        except Exception as e:
            logger.error(f"Gagal mengirim pesan ringkasan: {e}")
    ...
    if error_log:
        ...
        with open(f"{error_dir}/{client.me.id}_errors.txt", "w") as f:
            f.write("\n".join(error_log))
...
async def bcerror_cmd(client, message):
    ...
    try:
        ...
        try:
            ...
            message_text = f"<b>📋 Error Logs (from fallback):</b>\n\n<blockquote expandable>{content}</blockquote>"
...
async def addbl_cmd(client, message):
    ...
    try:
        ...
        text = f"{em.gagal}**chat_id must be in the form of numbers!**"
        return await pp.edit(text) if pp else await message.reply(text)
    ...
    if chat_id in blacklist:
        text = f"{em.gagal}**`{name}` already in the blacklist-Gcast!**"
        return await pp.edit(text) if pp else await message.reply(text)
        
    await dB.add_to_var(client.me.id, "BLACKLIST_GCAST", chat_id)
    
    text = f"{em.sukses}<b>Successfully adding `{name}` into blacklists</b>"
    return await pp.edit(text) if pp else await message.reply(text)
