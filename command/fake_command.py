...
async def mail_cmd(client, message):
    ...
    try:
        if message.command[1] == "gen":
            url = f"https://api.maelyn.eu/api/tools/tempmail/generate?apikey={API_MAELYN}"
            ...
            if len(message.command) > 2:
                ...
                url = f"https://api.maelyn.eu/api/tools/tempmail/inbox?id_inbox={ids}&apikey={API_MAELYN}"
                response = await Tools.fetch.get(url)
                if response.status_code == 200:
                    data = response.json()["result"]
                ...
                url = f"https://api.maelyn.eu/api/tools/tempmail/inbox?id_inbox={str(ids)}&apikey={API_MAELYN}"
                response = await Tools.fetch.get(url)
                if response.status_code == 200:
                    data = response.json()["result"]
                    inbox_data = data["inbox"]
